...
public class ThumbReconcileJob {
    ...
    public void run() {
        ...
        String pattern = RedisConstant.USER_THUMB_KEY_PREFIX + "*";
        try (Cursor<String> cursor = redisTemplate.scan(ScanOptions.scanOptions().match(pattern).count(1000).build())) {
            while (cursor.hasNext()) {
                String key = cursor.next();
                Long userId = Long.valueOf(key.replace(RedisConstant.USER_THUMB_KEY_PREFIX, ""));
                userIds.add(userId);
            }
        }
        ...
        userIds.forEach(userId -> {
            Set<Long> redisBlogIds = redisTemplate.opsForHash().keys(RedisConstant.USER_THUMB_KEY_PREFIX + userId).stream().map(obj -> Long.valueOf(obj.toString())).collect(Collectors.toSet());
        ...
        log.info("对账任务完成，耗时 {}ms", DateUtil.current() - startTime);
    }
